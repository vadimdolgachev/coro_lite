...
namespace CoroLite {
    ...
    CancellationToken::CancellationToken(std::shared_ptr<State> s) : state(std::move(s)) {
    }

    CancellationTokenSource::CancellationTokenSource() : state(std::make_shared<CancellationToken::State>()) {
    }
