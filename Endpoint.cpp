...
size_t std::hash<CoroLite::Endpoint>::operator()(const CoroLite::Endpoint &endpoint) const noexcept {
    ...
    if (endpoint.isIPv4()) {
        ...
        const auto &ipv4 = std::get<CoroLite::Endpoint::IPv4Addr>(endpoint.storage);
        const uint32_t ip = ntohl(ipv4.sin_addr.s_addr);
        return portHash ^ typeHash ^ ip4Hasher(ip);
    }
    if (endpoint.isIPv6()) {
        constexpr std::hash<uint64_t> ip6Hasher;
        const auto &ipv6 = std::get<CoroLite::Endpoint::IPv6Addr>(endpoint.storage);
    ...
    if (endpoint.isHostname()) {
        ...
        const auto &hostname = std::get<CoroLite::Endpoint::HostnameAddr>(endpoint.storage);
